...
from .schemas import PointCreate  # <-- IMPORTA EL NUEVO ESQUEMA

router = APIRouter(prefix="/points", tags=["points"])

@router.get("/", response_model=list[Point])
async def get_points(map_id: str | None = None): # Pydantic v1 compatible con str | None
    return await list_points(map_id)

@router.post("/", response_model=Point, status_code=201)
async def new_point(point_data: PointCreate):  # <-- USA EL ESQUEMA PointCreate
